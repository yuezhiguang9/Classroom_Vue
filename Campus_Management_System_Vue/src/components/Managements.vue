    ...
    <el-container>
         <!-- 侧边导航栏 -->
          <el-aside width="220px" class="aside-container">
            ...
            >
              <el-menu-item index="/mgr/man" v-if="data.user.role === 'MGR'" class="menu-item">
                <el-icon><User /></el-icon>
                <span>教室管理员系统</span>
              </el-menu-item>
              
              <el-menu-item index="/mgr/state" v-if="data.user.role === 'MGR'" class="menu-item">
                <el-icon><RefreshLeft /></el-icon>
                <span>教室状态管理</span>
              </el-menu-item>
              
              <el-menu-item index="/mgr/audit" v-if="data.user.role === 'MGR'" class="menu-item">
...
const data = reactive({
  user: JSON.parse(localStorage.getItem('class_mgr') || '{"id":1,"name":"教室管理员","role":"MGR"}')
})



data.user = {id: 1, name: '教室管理员', role: 'MGR'}

const updateUser = () => {
  data.user = JSON.parse(localStorage.getItem('class_mgr') || '{}')
}

const logout = () => {
  ElMessage.success('退出成功')
  localStorage.removeItem('class_mgr')
  router.push('/login')
}
</script>
