<template>
  <div class="font-inter bg-gray-50 min-h-screen flex flex-col">
    ...
    <main class="flex-grow flex items-center justify-center px-4 py-8">
      <div class="login-card w-full max-w-md transform transform transition-all duration-300 hover:shadow-lg">
        <!-- 标题与图标水平排列 -->
        ...
        <div class="identity-grid mb-8">
          ...
          >
            <i class="fa fa-user-circle-o text-xl mb-1" :class="selectedIdentity === 'student' ? 'text-primary' : 'text-gray-500'"></i>
            <span class="text-sm font-medium">学生</span>
          </div>
          <div 
            class="identity-option scale-hover"
            :class="{ 'identity-active': selectedIdentity === 'secretary' }"
            @click="selectIdentity('secretary')"
          >
            <i class="fa fa-briefcase text-xl mb-1" :class="selectedIdentity === 'secretary' ? 'text-primary' : 'text-gray-500'"></i>
            <span class="text-sm font-medium">教秘</span>
          </div>
          <div 
            class="identity-option scale-hover"
            :class="{ 'identity-active': selectedIdentity === 'classroomAdmin' }"
            @click="selectIdentity('classroomAdmin')"
          >
            <i class="fa fa-cog text-xl mb-1" :class="selectedIdentity === 'classroomAdmin' ? 'text-primary' : 'text-gray-500'"></i>
            <span class="text-sm font-medium">教室管理员</span>
          </div>
          <div 
            class="identity-option scale-hover"
            :class="{ 'identity-active': selectedIdentity === 'superAdmin' }"
            @click="selectIdentity('superAdmin')"
          >
            <i class="fa fa-shield text-xl mb-1" :class="selectedIdentity === 'superAdmin' ? 'text-primary' : 'text-gray-500'"></i>
        ...
        <form class="form-space" @submit.prevent="handleLogin">
          ...
          <button 
            type="submit" 
            class="login-btn mt-4 scale-hover"
            :disabled="isLoading"
          >
            <span v-if="!isLoading">登录系统</span>
            <span v-if="isLoading">
              <i class="fa fa-spinner fa-spin mr-1"></i>登录中...
            </span>
          </button>
        </form>
      </div>
    ...
    async handleLogin() {
      ...
      try {
        ...
        if (code === 200) {
          ...
          storage.setItem('currentUser', JSON.stringify({
          username: data.userInfo.username, // 从后端返回的userInfo中获取
          identity: this.selectedIdentity,
          name: data.userInfo.name, // 匹配后端返回的姓名字段
          college: data.userInfo.college || '' // 兼容可能不存在的字段
          }));
