			...
			<div class="search-card">
				<el-form :model="data.searchForm" inline>
					<el-form-item label="星期" style="width: 200px">
						<el-select
							v-model="data.searchForm.res_day_of_week"
							placeholder="请选择星期"
							clearable>
					...
					<el-form-item label="楼栋名称" style="width: 200px">
						<el-select
							v-model="data.searchForm.building_id"
							placeholder="请选择楼栋"
							clearable>
							<el-option
					...
					<el-form-item label="教室类型" style="width: 200px">
						<el-select
							v-model="data.searchForm.room_type"
							placeholder="请选择教室类型"
							clearable>
					...
					<el-form-item label="教室状态" style="width: 200px">
						<el-select
							v-model="data.searchForm.room_status"
							placeholder="请选择教室状态"
							clearable>
			...
			<div class="content-card">
				...
				<el-table
					...
					<el-table-column label="节次" align="center">
						<template #default="scope">
							{{
								Array.isArray(scope.row.period)
									? scope.row.period.join(",")
									: scope.row.period
							}}
						</template>
					</el-table-column>
					<el-table-column label="预约" align="center">
						<template #default="scope">
							<el-button
								v-if="
									(scope.row.room_status || scope.row.apply_status) === '空闲'
								"
								type="primary"
								size="small"
		...
		<el-dialog
			...
			<el-form
				...
				<el-form-item label="教室编号" prop="room_num">
					<el-input v-model="reservationForm.room_num" disabled />
				</el-form-item>
				...
				<el-form-item v-if="Object.keys(selectedPeriodsTime).length > 0">
					<div class="period-time-info">
						...
						<div class="period-time-list">
							<div
								v-for="(time, period) in selectedPeriodsTime"
								:key="period"
								class="period-item">
...
<script setup>
	...
	const formRules = reactive({
		user_account: [{ required: true, message: "请输入学号", trigger: "blur" }],
	...
	const submitReservation = async () => {
		try {
			...
			if (!dateRegex.test(submitData.use_date)) {
				ElMessage.error(
					`使用日期格式错误，必须为"YYYY-MM-DD HH:mm:ss"格式（如2025-09-12 12:00:00）`
				);
				return;
			}
			...
			if (
				!Number.isInteger(submitData.person_count) ||
				submitData.person_count < 1
			) {
			...
			const res = await axios.post(
				"http://localhost:8080/user/submitClassroomApply",
				submitData,
				{
					headers,
				}
			...
			if (res.data && res.data.code === 200) {
				...
				ElMessage.error(
					res.data?.message || "预约申请提交失败，服务器拒绝了请求"
				);
			}
		} catch (error) {
	...
	const load = async () => {
		...
		try {
			...
			if (data.searchForm.room_type)
				params.room_type = data.searchForm.room_type;
			if (data.searchForm.room_status)
				params.room_status = data.searchForm.room_status;
			if (data.searchForm.res_week) params.res_week = data.searchForm.res_week;
			if (data.searchForm.res_day_of_week)
				params.res_day_of_week = data.searchForm.res_day_of_week;
			...
			const res = await axios.get(
				"http://localhost:8080/user/selectClassroom",
				{
					headers,
					params,
				}
