        ...
        >
          ...
          <el-table-column label="预约" align="center">
            <template #default="scope">
              ...
              <span v-else class="not-available" style="color: #909399; cursor: not-allowed">
                不可预约
              </span>
            </template>
          </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="使用日期" prop="use_date">
          <el-input v-model="reservationForm.use_date" disabled placeholder="选择节次后自动生成" />
        </el-form-item>
...
<script setup>
  ...
  const submitReservation = async () => {
    try {
      ...
      const res = await axios.post("http://localhost:8080/user/submitClassroomApply", submitData, {
        headers,
      });
      ...
      if (error.name === "ValidationError") {
        ...
        if (error.response) {
          ...
          ElMessage.error(
            `提交失败: ${error.response.data.msg || `服务器返回错误 (${error.response.status})`}`
          );
        } else {
